...
const { attrs: validationAttrs } = useValidation({
  source: rawIpAddress,
  rules: [{ message: t('tools.ipv4-address-converter.invalidAddress'), validator: ip => isValidIpv4({ ip }) }],
});
</script>
...
<template>
  <div>
    ...
    <input-copyable
      ...
      :placeholder="t('tools.ipv4-address-converter.correctAddressPlaceholder')"
    />
  </div>
</template>
